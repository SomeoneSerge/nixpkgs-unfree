...
let
  ...
  inherit (utils) maybeBuildable isSupportedCuPackage;
  ...
  extraPackages = neverBreakExtra ++ [
    ...
    [
      "deepin"
      "image-editor"
    ] # Failed in https://github.com/NixOS/nixpkgs/pull/233581
    [ "ffmpeg-full" ]
    [ "gimp" ]
    [
      "gst_all_1"
      ...
      supported = builtins.concatMap (
        ...
        if mbSupported == [ ] then
          ...
          [
            {
              inherit jobName;
              package = (builtins.head mbSupported);
            }
          ]
      ) matrix;
      kvPairs = builtins.map ({ jobName, package }: lib.nameValuePair jobName package) supported;
  ...
  neverBreak =
    let
      ...
      cuPackages = lib.attrNames (
        lib.filterAttrs (
          name: value:
          (isSupportedCuPackage name value)
          || builtins.elem name [
            "saxpy"
            "cuda-samples"
            "cuda_nvcc"
      ...
      matrix = neverBreakExtra ++ builtins.map mkPath (matrixPy ++ matrixCu);
      ...
      kvPairs = builtins.map ({ jobName, package }: lib.nameValuePair jobName package) jobs;
    in
